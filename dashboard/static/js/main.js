...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (stateTableEl && window.all_travel) {
      ...
      const tableNode = d3TableWithControls(data, {
        searchKey: "name", // adjust to a real column name if different
        pageSize: 25,
      });
  ...
  // d3TableWithControls(data, { searchKey, pageSizes, pageSize })
  function d3TableWithControls(
    data,
    { searchKey = "name", pageSizes = [10, 25, 50, 100], pageSize = 25 } = {}
  ) {
    // --- make sure we include ALL columns seen in any row ---
    ...
    ctrTop.append("label").text(`Search ${searchKey}:`);
    const searchInput = ctrTop
      .append("input")
      .attr("type", "text")
      .attr("class", "d3-input")
    ...
    function update() {
      ...
      infoTop.text(`${rows.length ? start + 1 : 0}–${end} of ${rows.length}`);
      infoBot.text(
